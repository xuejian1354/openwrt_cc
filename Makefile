...
ifneq ($(CONFIG_TARGET_BRCM6848),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/lib/libuClibc-1.0.12.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libuClibc-1.0.12.so libc.so.1; ln -sf ld-uClibc-1.0.12.so ld-uClibc.so.1; ln -sf ld-uClibc.so.1 ld-uClibc.so.0;
endif
ifneq ($(or $(CONFIG_TARGET_BRCM6858),$(CONFIG_TARGET_BRCM6846)),)
...
ifneq ($(CONFIG_TARGET_BRCM6856),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/usr/arm-buildroot-linux-gnueabi/sysroot/lib/libc-2.26.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libc-2.26.so libc.so.0; ln -sf ld-2.26.so ld.so.1; ln -sf ld-2.26.so ld.so.0; ln -sf ld.so.0 ld.so;
endif
...
ifneq ($(CONFIG_TARGET_ZXIC2791V1),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/arm-buildroot-linux-gnueabi/sysroot/lib/libc-2.22.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libc-2.22.so libc.so.6; ln -sf ld-2.22.so ld-linux.so.3;
endif
...
ifneq ($(CONFIG_TARGET_ZTE279131),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/arm-sanechips-linux-gnueabihf/sysroot/lib/libc-2.26.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libc-2.26.so libc.so.6;
endif
ifneq ($(CONFIG_TARGET_CA8277),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/lib/libc-2.23.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libc-2.23.so libc.so.6;
endif
...
ifneq ($(CONFIG_TARGET_hi5662),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/lib/libc-2.22.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libc-2.22.so libc.so.6;
endif
...
ifneq ($(CONFIG_TARGET_MT7580),)
	cp $(CONFIG_TOOLCHAIN_ROOT)/mipsel-buildroot-linux-uclibc/sysroot/lib/libuClibc-0.9.33.2.so $(TARGET_DIR)/lib
	cd $(TARGET_DIR)/lib;ln -sf libuClibc-0.9.33.2.so libc.so.0; ln -sf ld-uClibc-0.9.33.2.so ld.so.1; ln -sf ld-uClibc-0.9.33.2.so ld-uClibc.so.0; ln -sf ld-uClibc.so.0 ld-uClibc.so;
endif
#fixme
