...
demo_interface_get_info(GDBusInterfaceSkeleton *skeleton)
{
	...
	static GDBusInterfaceInfo interface_info = {
		-1,
		(gchar *) "com.upointech.demoappv31.Config.section",
		NULL,
		NULL,
...
static void _get_uci_section_val(const char *section_type, int idx, const char *opt_name, char *opt_val)
{
	...
	sprintf(cmd, "/sbin/uci get demoappv3.@section[%d].%s", idx, opt_name);
	fp = popen(cmd, "r");
		...
		gpointer               user_data)
{
	...
	if (g_strstr_len(property_name, -1, "key")) {
		...
		sprintf(cmd, "/sbin/uci set demoappv3.@section[%d].%s=%s", self->number, property_name, val);
		system(cmd);
		system("/sbin/uci commit demoappv3");
	}
...
static const unsigned char config_global_xml[] =
	"<node>"
	"  <interface name='com.upointech.demoappv31.Config.global'>"
...
static const unsigned char config_section_xml[] =
	"<node>"
	"  <interface name='com.upointech.demoappv31.Config.section'>"
									  ...
									  GVariant           **outargs)
{
	...
	if (g_strcmp0(method_name, "Get") == 0) {
		...
		if (section_name)
			sprintf(cmd, "uci get demoappv3.%s.%s", section_name, key);
		else if (section_type)
			sprintf(cmd, "uci get demoappv3.@%s[%d].%s", section_type, section_idx, key);
		...
		*outargs = g_variant_new("(v)", g_variant_new("s", line));
		...
		if (g_strcmp0(method_name, "Set") == 0) {
			...
			if (section_name)
				sprintf(cmd, "/sbin/uci set demoappv3.%s.%s=%s", section_name, key, val);
			else if (section_type)
				sprintf(cmd, "/sbin/uci set demoappv3.@%s[%d].%s=%s", section_type, section_idx, key, val);
										...
										void          *userdata)
{
	...
	sprintf(cmd, "uci get demoappv3.@section[%d].%s", idx, prop);
	fp = popen(cmd, "r");
									   ...
									   void          *userdata)
{
	...
	sprintf(cmd, "/sbin/uci get demoappv3.global.%s", prop);
	fp = popen(cmd, "r");
...
static int demoappv3_restore(void)
{
	CtSgwLog(LOG_NOTICE, "restore factory\n");
	return 0;
}

static char *demoappv3_postmsg(char *msg)
{
	//msg can be any format, as long as caller have the protocol with callee
	g_print("PostMsg: %s\n", msg);
	return "this is msg from demoappv3";
}
...
static void demo_register_methods(CtSgwDBusNodeInfo_t *intro, CtSgwDBusNodeInfo_t *config_g)
{
	...
	fp = popen("/sbin/uci show demoappv3 | grep '=section' | wc -l", "r");
	...
	registration_id = CtSgwDBusRegisterObject("/com/upointech/demoappv31",
					  intro->interfaces[0],
					  &interface_vtable,
					  NULL);
	g_assert(registration_id > 0);

	registration_id = CtSgwDBusRegisterObject("/com/upointech/demoappv31/Config/global/global",
	...
	for (i = 0; i < cnt; i++) {
		sprintf(buf, "/com/upointech/demoappv31/Config/section/%d", i + 1);
#if 0
...
int main(int argc, char *argv[])
{
	...
	CtSgwAppMgtCallbacks_t app_cbs = {
		...
		.restore_factory = demoappv3_restore,
		.post_msg = demoappv3_postmsg
	};

	CtSgwLogOpen(LOG_USER, "demoappv3");
	CtSgwLog(LOG_NOTICE, "start\n");

	//test if /opt/apps/demoappv3/files/libdemo.so is available
	...
	msg = CtSgwPostMsg("testappv3", "hello testappv3");
	if (msg) {
		CtSgwLog(LOG_NOTICE, "testappv3 retmsg: %s", msg);
		g_free(msg);
	}
	...
	ret = CtSgwDBusStartService("com.upointech.demoappv31");
	if (ret < 0) {
