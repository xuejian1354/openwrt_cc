...
typedef char *(*CtSgwMgtStatsCallback)(void);
...
typedef struct {
    ...
    CtSgwMgtStatsCallback get_stats;
 ...
 *          timeout  <IN>:    the timeout in milliseconds, -1 to use the default timeout 25s or G_MAXINT for no timeout
 *
 * Return: 0 on success, otherwise failed.
 *
 */
