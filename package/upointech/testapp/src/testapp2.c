...
char *uds_file = "/uds/testappv3/testappv3.uds";
...
static const unsigned char intro_xml[] =
	"<node>"
	"  <interface name='com.upointech.testappv31.intf2'>"
	"    <method name='Config'>"
				   ...
				   void                *user_data)
{
	...
	if (g_strcmp0(method_name, "ReadOption") == 0) {
		...
		*outargs = g_variant_new("(s)", str);
		...
		//printf("%d, uci add\n", __LINE__);
		system("uci add testappv3 sec1");
		//printf("%d, uci rename\n", __LINE__);
		system("uci rename testappv3.@sec1[0]=name1");
		//printf("%d, uci set\n", __LINE__);
		system("uci set testappv3.@sec1[0].key1=val1");
		//printf("%d, uci commit\n", __LINE__);
		system("uci commit testappv3");
#endif
								 ...
								 GVariant           **outargs)
{
	...
	if (g_strcmp0(method_name, "Get") == 0) {
		...
		if (section_name)
			sprintf(cmd, "uci get testappv3.%s.%s", section_name, key);
		else if (section_type)
			sprintf(cmd, "uci get testappv3.@%s[%d].%s", section_type, section_idx, key);
		fp = popen(cmd, "r");
		if (fp) {
			fgets(line, 64, fp);
			len = strlen(line);
		...
		*outargs = g_variant_new("(v)", g_variant_new("s", line));
		...
		if (section_name)
			sprintf(cmd, "uci set testappv3.%s.%s=%s", section_name, key, val);
		else if (section_type)
			sprintf(cmd, "uci set testappv3.@%s[%d].%s=%s", section_type, section_idx, key, val);
...
static void test_register_methods(CtSgwDBusNodeInfo_t *intro, CtSgwDBusNodeInfo_t *config_g)
{
	...
	registration_id = CtSgwDBusRegisterObject("/com/upointech/testappv31",
					  intro->interfaces[0],
					  &interface_vtable,
					  NULL);
	...
	manager = g_dbus_object_manager_server_new("/com/upointech/testapp1v3/Config");

	skeleton = g_dbus_object_skeleton_new("/com/upointech/testappv31/Config/global/global");
...
int main(int argc, char *argv[])
{
	...
	CtSgwLogOpen(LOG_USER, "testappv3");

	CtSgwLog(LOG_NOTICE, "start\n");
	
	ret = CtSgwDBusStartService("com.upointech.testappv31");
	if (ret < 0) {
	...
	system("cat /etc/resolv.conf >/data/testappv3.dns");
