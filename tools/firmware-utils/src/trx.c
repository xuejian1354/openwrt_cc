...
struct trx_header {
	...
	char fwver[8];
	char operator[4];		/* CT/CU */
	char chiptype[12];
	char vendor[12];
	uint32_t revision;           /* 52-56 bytes */
	uint32_t reserved[14];   /* 57-112 bytes */
...
void usage(void)
{
	...
	fprintf(stderr, " trx [-2] [-o outfile] [-m maxlen] [-v fwver] [-a align] [-b absolute offset] [-x relative offset]\n");
...
int main(int argc, char **argv)
{
	...
	memset(p, 0, sizeof(struct trx_header));
	...
	while ((c = getopt(argc, argv, "-:2o:m:v:p:c:d:r:a:x:b:f:A:F:")) != -1) {
		switch (c) {
			case '2':
...
#endif				
				cur_len += n;
				fprintf(stderr, "***trx, %d, curlen = %d\n", __LINE__, cur_len);
				append = 0;

				break;
			case 'v':
				memcpy(p->fwver, optarg, strlen(optarg));
				break;
			case 'p':
				memcpy(p->operator, optarg, strlen(optarg));
				break;
			case 'c':
				memcpy(p->chiptype, optarg, strlen(optarg));
				break;
			case 'd':
				memcpy(p->vendor, optarg, strlen(optarg));
				break;
			case 'r':
				e = optarg;
				p->revision = STORE32_LE(atoi(e + 1));
				break;
			case 'o':
				ofn = optarg;
...
#define ROUND 0x1000
	...
	fprintf(stderr, "***trx, %d, crc32 = 0x%x, len = 0x%x, flag_version = 0x%x fwver %s operator %s vendor %s chiptype %s revision %d offsets0 = %d\n", 
		__LINE__, p->crc32, p->len, p->flag_version, p->fwver, p->operator, p->vendor, p->chiptype, p->revision, p->offsets[0]);
#if 0
